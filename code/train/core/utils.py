...
def mat2df_mimic(data):
    for popped in ['Age','BMI','Gender','Height','Weight',]:
        data.pop(popped)
    # convert to dataframe
    df = pd.DataFrame()
    
    df['patient'] = np.squeeze(np.array(data['Subject']))
    df['trial'] = np.nan
    df['SP'] = data['SBP']
    df['DP'] = data['DBP']
    # df['ecg'] = list(data['Signals'][:,0,:]) # ECG signal
    df['signal'] = list(data['Signals'][:,1,:]) # PPG signal
    df['abp_signal'] = list(data['Signals'][:,2,:]) # abp signal
    return df
...
def to_group(pred, true, config, normalizer):
    ...
    #pred_sp_crisis = []; 
    pred_sp_hyper2 = []; pred_sp_prehyper = []; pred_sp_normal = []; pred_sp_hypo = [];
    #pred_dp_crisis = []; 
    pred_dp_hyper2 = []; pred_dp_prehyper = []; pred_dp_normal = []; pred_dp_hypo = [];
    #true_sp_crisis = []; 
    true_sp_hyper2 = []; true_sp_prehyper = []; true_sp_normal = []; true_sp_hypo = [];
    #true_dp_crisis = []; 
    true_dp_hyper2 = []; true_dp_prehyper = []; true_dp_normal = []; true_dp_hypo = [];
...
def remove_outlier(list_of_df):
    ...
    for df in list_of_df:
        df_ = df[(80<df.SP) & (df.SP<180) & (40<df.DP) & (df.DP<120)].reset_index() # Outlier original remover
...
def group_count(list_of_df):
    ...
    dic = dict()
    for idd, df in enumerate(list_of_df):
        for ids,bp in enumerate(bp_group):
            dic[f"{bp}"] = (df['group']==ids).sum()
            df_ = pd.DataFrame([dic])
        if idd == 0:
            dff = df_.copy()
        else:
            dff = pd.concat([dff, df_], axis=0)    
    dff.index.name = "Fold" 
    print(dff)
    print("##"*20)
...
def pulsedb_group_shot(all_split_df, n=5): # df = list of df : [test, val, train]
    all_split_df = remove_outlier(all_split_df)
    all_split_df = group_annot(all_split_df)
    
    def recursive_sampling(all_split_df, n=5, max_attempts=10):
        attempts=0
        while attempts < max_attempts:
            try:
                test_df, val_df, train_df = all_split_df
                shot_train = group_shot(train_df, n=n)
                patient_list = shot_train['patient'].unique()
                val_same_patient = val_df[val_df['patient'].isin(patient_list)]
                shot_val = group_shot(val_same_patient, n=5) # Fixed 
                shot_test = group_shot(test_df, n=5) # Fixed 
                return shot_test, shot_val, shot_train 
            except SomeSpecificException as e:
                print(f"Attempt {attempts + 1} failed: {e}")
                print("Lack of data in validation set. Trying again...")
                attempts += 1

        raise RuntimeError("Maximum attempts reached. Unable to sample the data.")
    return recursive_sampling(all_split_df, n=n, max_attempts=10)
